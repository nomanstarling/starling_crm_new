...
@php
    $columns = [
        'source' => ['index' => 1, 'visible' => true],
        'sub_source' => ['index' => 2, 'visible' => true],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Sub Sources
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('subSources.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('subSources.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['source']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSource" placeholder="Search by Source name"></th>
                        <th id="2" class="{{ $columns['sub_source']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSubSource" placeholder="Search by Sub Source name"></th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Sub Source Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('subSources.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <select name="source_id" id="source_id" class="form-control form-control-sm selectTwo" required>
                                        @if(count($sources) > 0)
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="name" class="form-label">Sub Source Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Sub Source Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[1, 'asc'], [2, 'asc']],
        ajax: {
            url: '{{ route('subSources.getSubSources') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        initComplete: function () {
            ...
            $('.searchSubSource').on('keyup', function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('subSources.getSubSources') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.sub_sources).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Sub Source');
            modalAction = '{{ route('subSources.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Sub Source');
            modalAction = '{{ route('subSources.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('subSources.edit', ['sub_source' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var sub_source = data.sub_source;

                    // Basic Details
                    $('#source_id').val(sub_source.source_id).trigger('change');
                    $('#name').val(sub_source.name);

                    // Populate change log
                    
                    var activities = sub_source.activities;
                    var tableRow = null;
    ...
    function confirmDelete(deleteUrl) {
        Swal.fire({
            text: "Are you sure you want to delete this Sub Source?",
            icon: "warning",
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subSources.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subSources.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subSources.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subSources.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
