...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Communities
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('communities.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('communities.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['country']['visible'] ? '' : 'd-none' }}">
                            <input type="text" class="form-control form-control-sm searchCountry" placeholder="Search by Country name" />
                        </th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Community Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('communities.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                ...
                                <!-- <div class="col-md-6 mb-3">
                                    ...
                                    <select name="country_id" id="country" class="form-control form-control-sm country">
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="name" class="form-label">Communtiy Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Community Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[2, 'asc'], [3, 'asc']],
        ajax: {
            url: '{{ route('communities.getCommunities') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        //         $.ajax({
        //             url: '{{ route('communities.getCommunities') }}',
        //             type: 'GET',
        //             data: {
        //                 startDate: startDate,
        //                 endDate: endDate,
        ...
        //                 dataTable.clear().rows.add(data.communities).draw();
        //             },
    ...
    $('#editModal').on('show.bs.modal', function (event) {
        ...
        if (action === 'create') {
            ...
            modalAction = '{{ route('communities.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Community');
            modalAction = '{{ route('communities.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('communities.edit', ['community' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    ...
                    // $('#country').val(community.country_id).trigger('change');
                    // $('#city').val(community.city_id).trigger('change');
                    $('#city').val(community.city_id).trigger('change');
                    $('#city').trigger('change');
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('communities.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('communities.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('communities.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('communities.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
