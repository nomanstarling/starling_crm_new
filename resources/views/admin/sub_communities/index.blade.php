...
@php
    $columns = [
        ...
        'sale' => ['index' => 5, 'visible' => true],
        'rent' => ['index' => 6, 'visible' => true],
        'archive' => ['index' => 7, 'visible' => true],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Sub Communities
        ...
        <div class="card-toolbar">
            <button class="btn btn-flex btn-primary btn-sm mr-1 editModal" data-bs-toggle="modal" data-bs-target="#editModal" data-action="create">
                <i class="ki-duotone ki-plus fs-3"></i>
                Add
            </button>

            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('subCommunities.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('subCommunities.index') }}?status=deleted">
                Deleted
            </a>
            ...
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        ...
                        <th id="5" class="{{ $columns['sale']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSaleCount" placeholder="Search by Sale" /></th>
                        <th id="6" class="{{ $columns['rent']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchRentCount" placeholder="Search by Rent " /></th>
                        <th id="7" class="{{ $columns['archive']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchArchiveCount" placeholder="Search by Archived" /></th>
                        <th id="8" class="{{ $columns['created_date']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCreateDate" placeholder="Search by Date" /></th>
                        <th id="9" class="{{ $columns['last_update']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDate" placeholder="Search by Date" /></th>
                        <th id="10" class="{{ $columns['actions']['visible'] ? '' : 'd-none' }}"></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Sub Community Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('subCommunities.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                ...
                                <div class="col-md-12 mb-3 form-group">
                                    <label for="name" class="form-label">Sub Communtiy Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Sub Community Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-white py-2">
                    <button type="submit" class="btn btn-primary btn-sm">Save Sub Community</button>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        ajax: {
            url: '{{ route('subCommunities.getSubCommunities') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            ...
            data: function (d) {
                ...
                // d.searchArchiveCount = $('.searchArchiveCount').val();
                // d.searchSaleCount = $('.searchSaleCount').val();
                // d.searchRentCount = $('.searchRentCount').val();
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        console.log('Inside show.bs.modal event');
        $('#editForm')[0].reset();
        //countrySelect.val(countries.find(country => country.name === defaultCountry).id).trigger('change');
        // Unselect city
        ...
        if (action === 'create') {
            modalTitle.text('Create Sub Community');
            modalAction = '{{ route('subCommunities.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Sub Community');
            modalAction = '{{ route('subCommunities.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('subCommunities.edit', ['sub_community' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var sub_community = data.sub_community;

                    // Basic Details
                    $('#name').val(sub_community.name);
                    $('#city').val(sub_community.city_id).trigger('change');
                    populateCommunities(sub_community.city_id, sub_community.community_id);
                    
                    //triggerCommunity(sub_community.community_id);
                    var activities = sub_community.activities;
                    var tableRow = null;
    ...
    function confirmDelete(deleteUrl) {
        Swal.fire({
            text: "Are you sure you want to delete this sub community?",
            icon: "warning",
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subCommunities.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subCommunities.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subCommunities.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('subCommunities.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#exportBtn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route('subCommunities.export') }}',
            type: 'POST',
            data: {
