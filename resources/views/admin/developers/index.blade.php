...
@php
    $columns = [
        'developer' => ['index' => 1, 'visible' => true],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Developers
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('developers.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('developers.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['developer']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDeveloper" placeholder="Search by Developer"></th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Developer Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('developers.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col-md-3">
                                    <div class="image-input image-input-outline" data-kt-image-input="true" style="background-image: url({{ asset('assets/media/svg/avatars/blank-dark.svg') }})">
                                        <div class="image-input-wrapper w-125px h-125px logoImage" style="background-image: url({{ asset('assets/media/svg/avatars/blank-dark.svg') }})"></div>
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Developer Name" required>
                                </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[2, 'asc'], [3, 'asc']],
        ajax: {
            url: '{{ route('developers.getDevelopers') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return '<div class="d-flex align-items-center">' +
                        '<div class="symbol symbol-50px symbol-md-70px"><img src="' + row.logo_image + '" alt="' + row.name + '" class="img-fluid rounded"> </div>' +
                        '<div class="ms-3">' +
        ...
        initComplete: function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('developers.getDevelopers') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.developers).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Developer');
            modalAction = '{{ route('developers.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Developer');
            modalAction = '{{ route('developers.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('developers.edit', ['developer' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    ...
                    var logoDiv = $('.logoImage');
                    logoDiv.css('background-image', 'url(' + developer.logoImage + ')');

                    // Populate change log
                    
                    var activities = developer.activities;
                    var tableRow = null;
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('developers.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('developers.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('developers.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('developers.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
