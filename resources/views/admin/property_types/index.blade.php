...
@php
    $columns = [
        'category' => ['index' => 1, 'visible' => true],
        'property_type' => ['index' => 2, 'visible' => true],
        'code' => ['index' => 3, 'visible' => true],
        'sale' => ['index' => 4, 'visible' => true],
        'rent' => ['index' => 5, 'visible' => true],
        'archive' => ['index' => 6, 'visible' => true],
        'created_date' => ['index' => 7, 'visible' => false],
        'last_update' => ['index' => 8, 'visible' => false],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Property Types
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('propertyTypes.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('propertyTypes.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['category']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCategory" placeholder="Search by Category"></th>
                        <th id="2" class="{{ $columns['property_type']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchType" placeholder="Search by Property Type"></th>
                        <th id="3" class="{{ $columns['code']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCode" placeholder="Search by Code" /></th>
                        <th id="4" class="{{ $columns['sale']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSaleCount" placeholder="Search by Sale" /></th>
                        <th id="5" class="{{ $columns['rent']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchRentCount" placeholder="Search by Rent " /></th>
                        <th id="6" class="{{ $columns['archive']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchArchiveCount" placeholder="Search by Archived" /></th>
                        <th id="7" class="{{ $columns['created_date']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCreateDate" placeholder="Search by Date" /></th>
                        <th id="8" class="{{ $columns['last_update']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDate" placeholder="Search by Date" /></th>
                        <th id="9" class="{{ $columns['actions']['visible'] ? '' : 'd-none' }}"></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Property Type Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('propertyTypes.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="category" class="form-label">City <span class="text-danger">*</span></label>
                                    <select name="category_id" id="category" class="form-control form-control-sm category selectTwo" required>
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="name" class="form-label">Property Type<span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Property Type" required>
                                </div>

                                <div class="col-md-6 mb-3 form-group">
                                    <label for="code" class="form-label">Code<span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="code" name="code" placeholder="Code" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[2, 'asc'], [3, 'asc']],
        ajax: {
            url: '{{ route('propertyTypes.getPropertyTypes') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'category.name', visible: columnVisibility['category']['visible'], },
            {
                data: 'name',
                visible: columnVisibility['property_type']['visible'],
            },
            {
                data: 'code',
                visible: columnVisibility['code']['visible'],
            },
            {
        ...
        initComplete: function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('propertyTypes.getPropertyTypes') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.property_types).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            ...
            modalAction = '{{ route('propertyTypes.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Property Type');
            modalAction = '{{ route('propertyTypes.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('propertyTypes.edit', ['property_type' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var property_type = data.property_type;

                    // Basic Details
                    $('#name').val(property_type.name);
                    $('#code').val(property_type.code);
                    $('#category').val(property_type.category_id).trigger('change');

                    // Populate change log
                    
                    var activities = property_type.activities;
                    var tableRow = null;
    ...
    function confirmDelete(deleteUrl) {
        Swal.fire({
            text: "Are you sure you want to delete this property type?",
            icon: "warning",
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyTypes.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyTypes.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyTypes.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyTypes.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
