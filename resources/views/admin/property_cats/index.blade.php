...
@php
    $columns = [
        ...
        'code' => ['index' => 2, 'visible' => true],
        'sale' => ['index' => 3, 'visible' => true],
        'rent' => ['index' => 4, 'visible' => true],
        'archive' => ['index' => 5, 'visible' => true],
        'created_date' => ['index' => 6, 'visible' => true],
        'last_update' => ['index' => 7, 'visible' => true],
        'actions' => ['index' => 8, 'visible' => true],
    ];
@endphp
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Property Categories
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('propertyCats.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('propertyCats.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['category']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCategory" placeholder="Search by Category name"></th>
                        <th id="2" class="{{ $columns['code']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCode" placeholder="Search by Code"></th>
                        <th id="3" class="{{ $columns['sale']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSaleCount" placeholder="Search by Sale" /></th>
                        <th id="4" class="{{ $columns['rent']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchRentCount" placeholder="Search by Rent " /></th>
                        <th id="5" class="{{ $columns['archive']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchArchiveCount" placeholder="Search by Archived" /></th>
                        <th id="6" class="{{ $columns['created_date']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCreateDate" placeholder="Search by Date" /></th>
                        <th id="7" class="{{ $columns['last_update']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDate" placeholder="Search by Date" /></th>
                        <th id="8" class="{{ $columns['actions']['visible'] ? '' : 'd-none' }}"></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Property Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('propertyCats.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Category Name" required>
                                </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[1, 'asc'], [2, 'asc']],
        ajax: {
            url: '{{ route('propertyCats.getPropertyCats') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        initComplete: function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('propertyCats.getPropertyCats') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.property_cats).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Property Category');
            modalAction = '{{ route('propertyCats.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Property Category');
            modalAction = '{{ route('propertyCats.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('propertyCats.edit', ['property_category' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var property_category = data.property_category;

                    // Basic Details
                    $('#name').val(property_category.name);
                    $('#code').val(property_category.code);

                    // Populate change log
                    
                    var activities = property_category.activities;
                    var tableRow = null;
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyCats.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyCats.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyCats.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('propertyCats.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
