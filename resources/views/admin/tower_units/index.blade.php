...
@php
    $columns = [
        ...
        'unit' => ['index' => 6, 'visible' => true],
        'sale' => ['index' => 7, 'visible' => true],
        'rent' => ['index' => 8, 'visible' => true],
        'archive' => ['index' => 9, 'visible' => true],
        'created_date' => ['index' => 10, 'visible' => false],
        'last_update' => ['index' => 11, 'visible' => false],
        'actions' => ['index' => 12, 'visible' => true],
    ];
@endphp
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Tower Units
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('towerUnits.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('towerUnits.index') }}?status=deleted">
                Deleted
            </a>
            ...
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        ...
                        <th id="6" class="{{ $columns['unit']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchUnit" placeholder="Search by Unit Name" /></th>
                        <th id="7" class="{{ $columns['sale']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSaleCount" placeholder="Search by Sale" /></th>
                        <th id="8" class="{{ $columns['rent']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchRentCount" placeholder="Search by Rent " /></th>
                        <th id="9" class="{{ $columns['archive']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchArchiveCount" placeholder="Search by Archived" /></th>
                        <th id="10" class="{{ $columns['created_date']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCreateDate" placeholder="Search by Date" /></th>
                        <th id="11" class="{{ $columns['last_update']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDate" placeholder="Search by Date" /></th>
                        <th id="12" class="{{ $columns['actions']['visible'] ? '' : 'd-none' }}"></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Tower Unit Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('towerUnits.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <select name="city_id" id="city" class="form-control form-control-sm city selectTwo" required>
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <select name="community_id" id="community" class="form-control form-control-sm community" required>
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <select name="sub_community_id" id="sub_community" class="form-control form-control-sm sub_community" required>
                                        <option value="">Select Sub Community</option>
                                    </select>
                                </div>

                                <div class="col-md-6 mb-3 form-group">
                                    <label for="tower" class="form-label">Tower <span class="text-danger">*</span></label>
                                    <select name="tower_id" id="tower" class="form-control form-control-sm tower" required>
                                ...
                                <div class="col-md-12 mb-3 form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Unit Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-white py-2">
                    <button type="submit" class="btn btn-primary btn-sm">Save Tower Unit</button>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        ajax: {
            url: '{{ route('towerUnits.getTowerUnits') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'tower.name', visible: columnVisibility['tower']['visible'], },
            {
                data: 'name',
                visible: columnVisibility['unit']['visible'],
            },
            {
        ...
        initComplete: function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                // Trigger an AJAX request to your Laravel controller
                $.ajax({
                    url: '{{ route('towerUnits.getTowerUnits') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        // Update DataTable with filtered data
                        dataTable.clear().rows.add(data.tower_units).draw();
                    },
                    error: function (xhr, status, error) {
                        console.error(xhr.responseText);
                    }
    ...
    function populateSubCommunities(cityId, community_id, selectedSubCommunityId = null) {
        var sub_communitySelect = $('#sub_community').select2();
        ...
        // Return the AJAX object to allow chaining
        return subCommunitiesAjax;
    }

    function populateTowers(cityId, community_id, sub_community_id, selected_id = null) {
        var selectElement = $('#tower').select2();
        selectElement.empty();
        //console.log('City id: ' + cityId + ' | Community id: ' + community_id + ' | sub community: ' + selected_ids);
        selectElement.html('<option value="">Select Tower</option>');
    ...
    $(document).on('change', '#sub_community', function() {
        var selectedCityId = $('#city').val();
        var selectedCommunituId = $('#community').val();
        var sub_community_id = $(this).val();
        populateTowers(selectedCityId, selectedCommunituId, sub_community_id);
    });
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Tower Unit');
            modalAction = '{{ route('towerUnits.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Tower Unit');
            modalAction = '{{ route('towerUnits.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('towerUnits.edit', ['unit' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var tower_unit = data.tower_unit;

                    // Basic Details
                    $('#name').val(tower_unit.name);
                    $('#city').val(tower_unit.city_id).trigger('change');

                    populateCommunities(tower_unit.city_id, tower_unit.community_id).done(function () {
                        populateSubCommunities(tower_unit.city_id, tower_unit.community_id, tower_unit.sub_community_id).done(function(){
                            populateTowers(tower_unit.city_id, tower_unit.community_id, tower_unit.sub_community_id, tower_unit.tower_id);
                        });
                    });

                    // Populate change log
                    
                    var activities = tower_unit.activities;
                    var tableRow = null;
    ...
    function confirmDelete(deleteUrl) {
        Swal.fire({
            text: "Are you sure you want to delete this tower unit?",
            icon: "warning",
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('towerUnits.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('towerUnits.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('towerUnits.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('towerUnits.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
