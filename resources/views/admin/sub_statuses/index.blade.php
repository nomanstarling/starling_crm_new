...
@php
    $columns = [
        'status' => ['index' => 1, 'visible' => true],
        'sub_status' => ['index' => 2, 'visible' => true],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} CRM Sub Statuses
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('crmSubStatuses.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('crmSubStatuses.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['status']['visible'] ? '' : 'd-none' }}">
                            <select name="statusVal" id="statusVal" class="searchStatus form-control form-control-sm">
                        ...
                        <th id="2" class="{{ $columns['sub_status']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchSubStatus" placeholder="Search by Sub Status name"></th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Sub Status Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('crmSubStatuses.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="status_id" class="form-label">Parent Status <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="name" class="form-label">Sub Status Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Sub Status Name" required>
                                </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[1, 'asc'], [2, 'asc']],
        ajax: {
            url: '{{ route('crmSubStatuses.getCrmSubStatuses') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        columns: [
            ...
            // },
            {
                ...
                render: function (data, type, row) {
                    // Checkbox column
                    if(data.crm_status.badge != null){
                        return '<span class="badge badge-primary" style="background: '+ data.crm_status.badge +';">' + data.crm_status.name + '</span>';
                    }
                    else{
                        return data.crm_status.name;
                    }
                },
                visible: columnVisibility['status']['visible'],
            },
        ...
        initComplete: function () {
            ...
            $('.searchSubStatus').on('keyup', function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('crmSubStatuses.getCrmSubStatuses') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Sub Status');
            modalAction = '{{ route('crmSubStatuses.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Sub Status');
            modalAction = '{{ route('crmSubStatuses.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('crmSubStatuses.edit', ['sub_status' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var sub_status = data.sub_status;

                    // Basic Details
                    $('#status_id').val(sub_status.status_id);
                    $('#name').val(sub_status.name);
                    $('#badge').val(sub_status.badge);

                    // Populate change log
                    
                    var activities = sub_status.activities;
                    var tableRow = null;
    ...
    function confirmDelete(deleteUrl) {
        Swal.fire({
            text: "Are you sure you want to delete this Sub Status?",
            icon: "warning",
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmSubStatuses.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmSubStatuses.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmSubStatuses.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmSubStatuses.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
