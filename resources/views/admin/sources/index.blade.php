...
@php
    $columns = [
        ...
        'created_date' => ['index' => 2, 'visible' => true],
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Sources
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('sources.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('sources.index') }}?status=deleted">
                Deleted
            </a>
            ...
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        ...
                        <th id="2" class="{{ $columns['created_date']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchCreateDate" placeholder="Search by Date" /></th>
                        <th id="3" class="{{ $columns['last_update']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchDate" placeholder="Search by Date" /></th>
                        <th id="4" class="{{ $columns['actions']['visible'] ? '' : 'd-none' }}"></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Source Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('sources.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Source Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[1, 'asc'], [2, 'asc']],
        ajax: {
            url: '{{ route('sources.getSources') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.sources).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Source');
            modalAction = '{{ route('sources.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Source');
            modalAction = '{{ route('sources.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('sources.edit', ['source' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('sources.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('sources.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('sources.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('sources.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
