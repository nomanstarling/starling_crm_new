...
@php
    $columns = [
        'owner' => ['index' => 1, 'visible' => true],
        'refno' => ['index' => 2, 'visible' => true],
        'whatsapp' => ['index' => 3, 'visible' => true],
...
<input type="hidden" name="update_perm" id="update_perm" value="{{ auth()->user()->can('owners_update') ? 'true' : 'false' }}">

<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} Owners
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('owners.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('owners.index') }}?status=deleted">
                Deleted
            </a>
            ...
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        ...
                        <th id="1" class="{{ $columns['owner']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchOwner input" placeholder="Search by Owner name, email, phone"></th>
                        <th id="2" class="{{ $columns['refno']['visible'] ? '' : 'd-none' }}">
                            <input type="text" class="form-control form-control-sm searchRefno input" placeholder="Search by Ref No" />
                        </th>
                        <th id="3" class="{{ $columns['whatsapp']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchWhatsapp input" placeholder="Search by Whatsapp" /></th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">Owner Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('owners.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                                ...
                                <div class="col-lg-10">
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3 mb-3 form-group">
                                            ...
                                            <input type="tel" class="form-control form-control-sm phone" id="whatsapp" name="whatsapp" placeholder="WhatsApp">
                                        </div>
                ...
                <div class="modal-footer bg-white py-2">
                    <button type="submit" class="btn btn-primary btn-sm">Save Owner</button>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[2, 'asc'], [3, 'asc']],
        ajax: {
            url: '{{ route('owners.getOwners') }}' + (status ? '?status=' + status : ''),
            type: 'POST',
            headers: {
    ...
    function handleModalShown(itemId, action) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Owner');
            modalAction = '{{ route('owners.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Owner');
            modalAction = '{{ route('owners.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('owners.edit', ['owner' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    var owner = data.owner;
                    imageDiv.css('background-image', 'url(' + owner.profile_image + ')');

                    // Basic Details
                    $('#modalRefNo').text(owner.refno);
                    $('#name').val(owner.name);
                    $('#title').val(owner.title).trigger('change');
                    $('#phone').val(owner.phone).trigger('change');
                    $('#whatsapp').val(owner.whatsapp).trigger('change');
                    $('#email').val(owner.email);
                    $('#dob').val(owner.dob);
                    $('#country_id').val(owner.country_id).trigger('change');
                    $('#address').val(owner.address);
                    $('#source_id').val(owner.source_id).trigger('change');
                    $('#company').val(owner.company);
                    $('#designation').val(owner.designation);
                    if(owner.city_id != null){
                        populateCities(owner.country_id, owner.city_id);
                    }
                    if(owner.sub_source_id != null){
                        populateSubSources('sub_source_id', 'modal', owner.source_id, owner.sub_source_id);
                    }

                    // Display Documents
                    displayDocuments(owner.documents);

                    displayNotes(owner.notes);
    ...
    $('#editForm').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                submitButton.prop('disabled', false).html('Save Owner');
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('owners.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('owners.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('owners.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('owners.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
