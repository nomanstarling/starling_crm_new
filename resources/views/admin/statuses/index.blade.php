...
@php
    $columns = [
        'status_name' => ['index' => 1, 'visible' => true],
        'type' => ['index' => 2, 'visible' => true],
        'created_date' => ['index' => 3, 'visible' => true],
        'last_update' => ['index' => 3, 'visible' => true],
        'actions' => ['index' => 4, 'visible' => true],
    ];
@endphp
<div class="card">
    <div class="card-header">
        <h2 class="card-title fw-bold">
            {{ isset($_GET['status']) ? ucfirst($_GET['status']) : 'Active' }} CRM Statuses
        ...
        <div class="card-toolbar">
            ...
            <a class="btn btn-flex btn-dark btn-sm mr-1" href="{{ route('crmStatuses.index') }}?status={{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'active' : 'inactive' }}">
                {{ isset($_GET['status']) && $_GET['status'] == 'inactive' ? 'Active' : 'Inactive' }}
            </a>

            <a class="btn btn-flex btn-danger btn-sm mr-1" href="{{ route('crmStatuses.index') }}?status=deleted">
                Deleted
            </a>
        ...
        <div class="tableDiv" id="tableDiv">
            <table class="table w-100 table-hover table-row-dashed" id="dataTable">
                ...
                <thead>
                    <tr id="filterHead">
                        <th id="0"></th>
                        <th id="1" class="{{ $columns['status_name']['visible'] ? '' : 'd-none' }}"><input type="text" class="form-control form-control-sm searchStatusName" placeholder="Search by Status name"></th>
                        <th id="2" class="{{ $columns['type']['visible'] ? '' : 'd-none' }}">
                            <select name="status_type" id="status_type" class="searchType form-control form-control-sm">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-grey">
            <div class="modal-header py-3 bg-white">
                <h5 class="modal-title" id="editModalLabel">CRM Status Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editForm" action="{{ route('crmStatuses.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="card-body py-4 px-5">
                            <div class="row mt-4">
                                <div class="col-md-6 mb-3 form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="name" name="name" placeholder="Status Name" required>
                                </div>
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="type" class="form-label">Status Type <span class="text-danger">*</span></label>
                                    <select name="type" id="type" class="type form-control form-control-sm" required>
                                        <option value="Listings">Listings</option>
                                        <option value="Leads">Leads</option>
                                    </select>
                                </div>

                                <div class="col-md-6 mb-3 form-group d-none" id="lead_type_div">
                                    <label for="lead_type" class="form-label">Lead Type <span class="text-danger">*</span></label>
                                    <select name="lead_type" id="lead_type" class="lead_type form-control form-control-sm">
                                ...
                                <div class="col-md-6 mb-3 form-group">
                                    <label for="badge" class="form-label">Badge Color</label>
                                    <input type="color" class="form-control form-control-sm" id="badge" name="badge">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var dataTable = new DataTable('#dataTable', {
        ...
        order: [[1, 'asc'], [2, 'asc']],
        ajax: {
            url: '{{ route('crmStatuses.getCrmStatuses') }}' + (status ? '?status=' + status : ''),
            type: 'GET',
            data: function (d) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    // Checkbox column
                    if(data.badge != null){
                        return '<span class="badge badge-primary" style="background: '+ data.badge +';">' + data.name + '</span>';
                    }
                    else{
                        return data.name;
                    }
                },
                visible: columnVisibility['status_name']['visible'],
            },
            {
                data: 'type',
        ...
        initComplete: function () {
            ...
            $('.searchStatusName').on('keyup', function () {
            ...
            $('.searchDate').on('apply.daterangepicker', function (ev, picker) {
                ...
                $.ajax({
                    url: '{{ route('crmStatuses.getCrmStatuses') }}',
                    type: 'GET',
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                    ...
                    success: function (data) {
                        ...
                        dataTable.clear().rows.add(data.statuses).draw();
                    },
    ...
    $('#editModal').on('shown.bs.modal', function (event) {
        ...
        if (action === 'create') {
            modalTitle.text('Create Status');
            modalAction = '{{ route('crmStatuses.store') }}';
        } else if (action === 'edit') {
            modalTitle.text('Edit Status');
            modalAction = '{{ route('crmStatuses.update', ':itemId') }}'.replace(':itemId', itemId);
        }
        ...
        if (action === 'edit') {
            $.ajax({
                url: '{{ route('crmStatuses.edit', ['status' => ':itemId']) }}'.replace(':itemId', itemId),
                type: 'GET',
                success: function(data) {
                    ...
                    $('#type').val(status.type).trigger('change');
                    if(status.lead_type && status.lead_type != null){
                        $('#lead_type').val(status.lead_type);
                    }
    ...
    $('#bulkDeleteBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmStatuses.bulkDelete') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkRestoreBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmStatuses.bulkRestore') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkActivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmStatuses.bulkActivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('#bulkDeactivateBtn').on('click', function () {
        ...
        if (selectedItems.length > 0) {
            performBulkAction('{{ route('crmStatuses.bulkDeactivate') }}', { item_ids: selectedItems });
        }
    });
    ...
    $('body').on('change', '#type', function () {
        if($(this).val() == 'Leads'){
            $('#lead_type_div').removeClass('d-none');
        }
        else{
            $('#lead_type_div').addClass('d-none');
        }
    });
